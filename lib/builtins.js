/*global -name*/
"use strict";
var exec = require('../exec');
var read = require('../read');
var write = require('../write');
var error = require('../error');
var slice = [].slice;

module.exports = {
  def: defForm,
  "set": setForm,
  ".": indexForm,
  λ: λForm,
  "?": triForm,
  if: ifForm,
  unless: unlessForm,
  and: andForm,
  or: orForm,
  not: notForm,
  print: printForm,
  list: listForm,
  read: read,
  write: write,
  exec: exec,
  escape: quoteForm,
  sleep: sleepForm,
  macro: macroForm,
  concat: concatForm,
  flat: flatForm,
  join: joinForm,
  shuffle: shuffleForm,
};

function joinForm(string, list) {
  if (!Array.isArray(list)) {
    throw error(list, __filename, "expected list");
  }
  return list.join(string);
}

function shuffleForm(list) {
  if (arguments.length !== 1 || !Array.isArray(list)) {
    throw "shuffle expects a single list";
  }
  list = list.slice();
  for (var i = 0, l = list.length; i < l; i++) {
    var j = Math.floor(Math.random() * l);
    var t = list[i];
    list[i] = list[j];
    list[j] = t;
  }
  return list;
}

function concatForm() {
  return flatForm(arguments);
}

function flatForm(lists) {
  var total = [];
  for (var i = 0; i < lists.length; i++) {
    var list = lists[i];
    if (!Array.isArray(list)) {
      throw error(list, __filename, "list expected", TypeError);
    }
    if (!list.length) continue;
    total.push.apply(total, list);
  }
  return total;
}

quoteForm.raw = true;
function quoteForm(item) {
  if (arguments.length !== 1) {
    throw "quote expects exactly 1 argument";
  }
  return item;
}

function* sleepForm(ms) {
  yield function (callback) {
    setTimeout(callback, ms);
  };
  return null;
}

defForm.raw = true;
function* defForm(id) {
  /*jshint validthis:true*/
  var body = slice.call(arguments, 1);
  if (Array.isArray(id)) {
    return yield* defForm.call(this, id[0], [λForm, id.slice(1)].concat(body));
  }
  var name = getVar(id);
  if (this.hasOwnProperty(name)) {
    throw error(id, "def", "Can't redefine local variable '" + name + "'", TypeError);
  }
  return (this[name] = yield* exec.apply(this, body));
}

setForm.raw = true;
function* setForm(id) {
  /*jshint validthis:true*/
  var body = slice.call(arguments, 1);
  if (Array.isArray(id)) {
    return yield* setForm.call(this, id[0], [λForm, id.slice(1)].concat(body));
  }
  var name = getVar(id);
  var scope = this;
  while (scope && !scope.hasOwnProperty(name)) scope = Object.getPrototypeOf(scope);
  if (!scope) {
    throw error(name, "set", "Can't set undefined variable '" + name + "'", TypeError);
  }
  return (scope[name] = yield* exec.apply(this, body));
}

function indexForm(list, index, value) {
  if (!Array.isArray(list)) {
    throw error(list, __filename, "list expected", TypeError);
  }
  if (arguments.length === 2) return list[index];
  if (arguments.length === 3) return (list[index] = value);
  throw "Index expects 2 or three arguments";
}



λForm.raw = true;
function λForm(ids) {
  /*jshint validthis:true*/
  var body = slice.call(arguments, 1);
  return makeLambda.call(this, getNames(ids), body);
}

// TODO: execute all macros up-front at compile time
macroForm.raw = true;
function* macroForm(ids) {
  /*jshint validthis:true*/
  var body = slice.call(arguments, 1);
  var action = makeLambda.call(this, getNames(ids.slice(1)), body);
  macro.raw = true;
  return yield* defForm.call(this, ids[0], macro);
  function* macro() {
    /*jshint validthis:true*/
    var out = yield* action.apply(this, arguments);
    // console.log("Generated", out.map(write).join(" "));
    return yield* exec.apply(this, out);
  }
}


function makeLambda(names, body) {
  /*jshint validthis:true*/
  var closure = this;
  λ.body = body;
  λ.names = names;
  return λ;
  function* λ() {
    var context = Object.create(closure);
    if (arguments.length !== names.length) {
      throw "Expected " + names.length + " arguments, but got " + arguments.length;
    }
    for (var i = 0; i < names.length; i++) {
      context[names[i]] = arguments[i];
    }
    return yield* exec.apply(context, body);
  }
}

function printForm() {
  console.log.apply(console, arguments);
  return null;
}

function listForm() {
  return slice.call(arguments);
}


triForm.raw = true;
function* triForm(condition, yes, no) {
  /*jshint validthis:true*/
  if (condition === undefined || yes === undefined) {
    throw "condition and yes path required in if";
  }
  var value = yield* exec.call(this, condition);
  if (value) return yield* exec.call(this, yes);
  return yield* exec.call(this, no || null);
}

ifForm.raw = true;
function* ifForm(condition) {
  /*jshint validthis:true*/
  var body = slice.call(arguments, 1);
  var value = yield* exec.call(this, condition);
  if (value) return yield* exec.apply(this, body);
  return null;
}

unlessForm.raw = true;
function* unlessForm(condition) {
  /*jshint validthis:true*/
  var body = slice.call(arguments, 1);
  var value = yield* exec.call(this, condition);
  if (!value) return yield* exec.apply(this, body);
  return null;
}

andForm.raw = true;
function* andForm() {
  /*jshint validthis:true*/
  var result = true;
  for (var i = 0; i < arguments.length; i++) {
    result = yield* exec.call(this, arguments[i]);
    if (!result) return result;
  }
  return result;
}

orForm.raw = true;
function* orForm() {
  /*jshint validthis:true*/
  var result = false;
  for (var i = 0; i < arguments.length; i++) {
    result = yield* exec.call(this, arguments[i]);
    if (result) return result;
  }
  return result;
}

function notForm(expression) {
  if (arguments.length !== 1) {
    throw "not expects exactly one argument";
  }
  return !expression;
}

function getNames(ids) {
  if (!Array.isArray(ids)) {
    throw error(ids, __filename, "Expected name list", SyntaxError);
  }
  return ids.map(getVar);
}

function getVar(item) {
  if (!item || typeof item !== "object" || !item.id) {
    throw error(item, __filename, "expected variable", SyntaxError);
  }
  return item.id;
}
